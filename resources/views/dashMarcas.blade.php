...
<div id="botaoCadUsuario">
    <a href="/cadastroMarcas">Cadastro de marcas</a>
</div>
            ...
            <div class="col-lx-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-description"> Todas as marcas cadastradas </p>
                            ...
                            <table class="table table-hover">
                                ...
                                @foreach ($marcas as $marca)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="editarMarca/<?= $marca->id ?>" id="view" class="button" >
                                                <i class="fa fa-eye fa-align-center" aria-hidden="true" style="font-size: 20px"></i>
                                            </a>
                                            <a  class="button" id="view"  onclick="deletarTurno('<?= $marca->id?>')">
...
<script>
    function deletarTurno(id) {
        let text = "Deseja realmente deletar a marca?";
        if (confirm(text) == true) {
            const link = document.getElementsByName("link")[0].value;

            const url = link+"/api/deletarMarca";
            $.ajax({
                url: url,
                type: 'POST',
                headers: {
                ...
                success: function () {
                    window.location.href = link+'/dashMarcas';
                    alert("Marca deletada com sucesso!!");
                },
                error: function () {
                    alert("Houve um problema ao deletar a marca!");
                },
            });
        }
    }
